define([
  ...
  var tree;
  window.tree = tree;
  var reloadTree = ko.observable(0);
  ...
  return plugins.make({
    ...
    route: function(url, breadcrumb) {
      ...
      if (url.parameters[0]){
        ...
        openFile({
          name: url.parameters[url.parameters.length-1],
          location: "/"+url.parameters.join("/")
        });
      } else if (!selectedDocument()){
        breadcrumb(all);
        loadUrl(url);
        // $.when(fs.browse(serverAppModel.location)).then(loadInTree);
        window.location.hash = "code/";
      }
    }

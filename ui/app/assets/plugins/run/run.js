  ...
  var runConsole = api.PluginWidget({
    ...
    init: function(parameters){
      ...
      this.haveActiveTask.subscribe(function(active) {
        if (active && self.runWithInspect())
          api.events.send({ 'type' : 'RunWithInspectStarted' });
        else if (active)
          api.events.send({ 'type' : 'RunStarted' });
        else
      ...
      this.inspectCompatible = model.snap.app.hasEcho;
      this.runWithInspect = ko.observable(true);
    ...
    doRunWithoutMainClassLoad: function(clearLogs) {
      ...
      if (self.runWithInspect()) {
        task.task = 'echo:' + task.task;
        // TODO: get trace port from elucidator running in activator
        task.params.tracePort = 28667;
      }

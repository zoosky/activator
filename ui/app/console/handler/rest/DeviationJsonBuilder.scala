...
class DeviationJsonBuilder extends JsonBuilderActor {
  ...
  def receive = {
    case r: DeviationResult => r.receiver ! Update(createJson(r.eventID, r.traces))
  }
}
...
object DeviationJsonBuilder {
  ...
  case class DeviationResult(receiver: ActorRef, eventID: UUID, traces: Seq[TraceEvent])
  ...
  def createJson(eventId: UUID, traces: Seq[TraceEvent]): JsObject =
    Json.obj(
      "type" -> "deviation",
      "data" ->
        Json.obj(
